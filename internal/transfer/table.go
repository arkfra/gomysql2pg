...
func (tb *Table) TableCreate(logDir string, tblName string, ch chan struct{}) {
	...
	// 使用goroutine并发的创建多个表
	var colTotal int
	pgCreateTbl := fmt.Sprintf(`create table "%s" (`, tblName)
	// 查询当前表总共有多少个列字段
	...
	err := tb.srcDb.QueryRow(colTotalSql).Scan(&colTotal)
	if err != nil {
		log.Error(err)
	}
	// 查询MySQL表结构
	...
	// 创建前先删除目标表
	dropDestTbl := fmt.Sprintf(`drop table if exists "%s" cascade`, tblName)
	if _, err = tb.destDb.Exec(dropDestTbl); err != nil {
	...
	if _, err = tb.destDb.Exec(pgCreateTbl); err != nil {
...
func (tb *Table) SeqCreate(logDir string) (result []string) {
	...
	for rows.Next() {
		...
		// 创建前先删除目标序列
		if _, err = tb.destDb.Exec(tb.dropSeqSql); err != nil {
			log.Error(err)
		}
		// 创建目标序列
		...
		if _, err = tb.destDb.Exec(tb.destSeqSql); err != nil {
		...
		if _, err = tb.destDb.Exec(tb.destDefaultSeq); err != nil {
...
func (tb *Table) IdxCreate(logDir string) (result []string) {
	...
	for rows.Next() {
		...
		if tb.Cfg().Distributed {
			if indexName == "PRIMARY" {
				if _, err = tb.destDb.Exec(alterDistributeSql); err != nil {
		...
		if _, err = tb.destDb.Exec(tb.destIdxSql); err != nil {
...
func (tb *Table) FKCreate(logDir string) (result []string) {
	...
	// 先获取有外键的表名
	sql := "select  table_name from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_SCHEMA =database();"
	//fmt.Println(sql)
	rows, err := tb.srcDb.Query(sql)
	if err != nil {
		log.Error(err)
	...
	for rows.Next() {
		...
		err := tb.srcDb.QueryRow(sql).Scan(&createSql) // 根据单个表获取外键拼接sql
		if err != nil {
			log.Error(err)
		}
		// 创建目标外键
		if createSql != "null" {
			...
			if _, err = tb.destDb.Exec(createSql); err != nil {
...
func (tb *Table) ViewCreate(logDir string) (result []string) {
	...
	sql := "select table_name,concat('create or replace view ',table_name,' as ',  replace(replace(replace(replace(VIEW_DEFINITION,'`',''),concat(table_schema,'.'),''),'convert(',''),'using utf8mb4)','')  ,';') create_view_sql from information_schema.VIEWS where TABLE_SCHEMA=database();"
	rows, err := tb.srcDb.Query(sql)
	if err != nil {
		log.Error(err)
	}
	...
	for rows.Next() {
		...
		if _, err = tb.destDb.Exec(tb.viewSql); err != nil {
...
func (tb *Table) TriggerCreate(logDir string) (result []string) {
	...
	var createSql string
	// 查询触发器，批量生成创建语句
	sql := "SELECT replace(lower(concat('create or replace trigger ',trigger_name,' ',action_timing,' ',event_manipulation,' on \"',event_object_table,'\" for each row as ',action_statement)),'#','-- ') FROM information_schema.triggers WHERE trigger_schema=database();"
	//fmt.Println(sql)
	rows, err := tb.srcDb.Query(sql)
	if err != nil {
		log.Error(err)
